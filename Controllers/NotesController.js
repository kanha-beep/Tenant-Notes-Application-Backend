...
export const allNotes = async (req, res, next) => {
  ...
  const adminPagination = await Notes.find({ ...query, tenant: req.user.tenant._id })
    .populate("tenant")
    .sort(sortOptions)
    .skip(skip)
    .limit(limit);
  const userPagination = await Notes.find({ ...query, tenant: req.user.tenant._id, user: req.user._id })
    .populate("tenant")
    .sort(sortOptions)
    .skip(skip)
    .limit(limit);
